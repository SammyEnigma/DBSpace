...
insert into  Questionnaire (QuestionnaireDate
			...
			, ReviewStatusID)
					...
					LEFT OUTER JOIN Person AS P on TQ.ChildID = P.HistoricChildID
					LEFT OUTER JOIN [QuestionnaireDataSource] AS QDS on TQ.Source = QDS.[QuestionnaireDataSourceName]
					RIGHT OUTER JOIN ReviewStatus AS RS ON RS.HistoricReviewStatusID = TQ.ReviewStatusCode
					LEFT OUTER JOIN Questionnaire AS Q ON TQ.QuestDate = Q.QuestionnaireDate and TQ.ChildID = Q.HistoricChildID
					where Q.QuestionnaireDate is null and Q.HIstoricCHildID is null
